'use strict';
{
    ...
    let stucked = false;
    let blocks = [];
    let mainHeight = droparea.height;
    ...
    function displayblock() {
        // 配列初期化
        blocks = [
            {color:'yellow', startX:25, startY:25, drawX:24, drawY:24, h:50, w:50},
            {color:'lightblue', startX:25, startY:25, drawX:24, drawY:24, h:25, w:100},
            {color:'blue', startX:35, startY:25, drawX:24, drawY:24, h:50, w:75},
            {color:'orange', startX:10, startY:50, drawX:24, drawY:24, h:50, w:75},
            {color:'green', startX:10, startY:50, drawX:24, drawY:24, h:50, w:50},
            {color:'red', startX:10, startY:25, drawX:24, drawY:24, h:50, w:50},
            {color:'purple', startX:35, startY:50, drawX:24, drawY:24, h:50, w:75}
        ];
    ...
    const Block = function(x, y, vy) {
        ...
        this.move = function() {
            ...
            this.y += this.vy;
            return blocks[Index2].h;
        }
        
        // ストップ判定
        this.checkCollision = function(bh) {
            // 一番下に来たらストップ
            if (this.y == mainHeight) {
    ...
    // X始点、Y始点、ブロック一辺サイズ
    let fallBlock = new Block(60, 0, 25);
    
    // 画面描画更新
    function update() {
        let bh = fallBlock.move();
        console.log(fallBlock);
        timeoutId = setTimeout(() => {
            update();
        }, 100);
        fallBlock.checkCollision(bh);
    }
    update();
